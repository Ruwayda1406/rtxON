...
void main() {
	...
	/*
	// Is this a fully transparent part of the surface?  If so, ignore this hit
	else// if (nextRand(PrimaryRay.rndSeed) > hit.matColor.w) //ignore some hits
	{
		PrimaryRay.isTransparent = true;
		if (PrimaryRay.accColor.w < 1.0)
		{
			vec3 diffColor = DiffuseShade(hit.pos, hit.normal, hit.matColor.xyz, hit.kd, hit.ks);
		//	diffColor.xyz *= hit.matColor.w;
			vec4 color = (1.0 - PrimaryRay.accColor.w) * vec4(diffColor, hit.matColor.w);
			// Front-to-back compositing
			PrimaryRay.accColor = color + PrimaryRay.accColor;
